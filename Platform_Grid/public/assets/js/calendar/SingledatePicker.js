	...
	$.fn.GridDefaultCalendar = (CalendarOPtions) => {
		...
		$.extend( RunCalen.prototype, {
			...
			calendarType: () => {
				...
				let SingleCalendar = $(settingsCal.tags.divTag, { class: 'SingleCalendar' }).appendTo('.calenPicker')
				...
				$.fn.RunTeleGridcalendar = function() {
					...
					$.fn.OpenCalendGl = () => {
						return this.on('click', '*', function(e, event) {
							...
							if(cls.indexOf('datepickerFrom') != -1) {

								// Open Calendar Date From
								datetimepicker('100%', '0', 'auto');
								...
								$('#datetimepicker_From').addClass('datepicker');
							}

							// Click Event for Calendar Date To
							else if(cls.indexOf('datepickerTo') != -1) {

								// Open Calendar Date to
								datetimepicker('100%', '167px', 'auto');

								// Removes Attribute Id
								$('.datepickerFrom').removeAttr('id');

								// Adds Attribute Id
								$(this).attr({ id: 'datepicker'});

								// Removes Class From BackEnd Input
								$('#datetimepicker_From').removeClass('datepicker');

								// Adds Class To BackEnd Input
								$('#datetimepicker_To').addClass('datepicker');
							}
							...
							if(cls.indexOf('dyItem') != -1) {
								...
								if($('#datepicker .dyItem').length == 0) {
									...
									$('#datepicker').html( NullHeader($(this).html()) + $('.ThisMonthTop .MonthNum').text() + $('.ThisYearTop').text());
									...
									$(this).clone().appendTo('#datepicker').removeClass('CalenTd today setDay');
									
									$('#datepicker .MandY_Floor').remove();
					...
					function datetimepicker(tops, lefts, rights) {

						$('.SingleCalendar').slideToggle(30).css({ top: tops, left: lefts, right: rights });
					}
